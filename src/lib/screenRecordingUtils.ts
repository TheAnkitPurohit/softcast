...
export const createThumbnailClientSide = async (
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const onLoaded = () => resolve()
      const onError = () => reject(new Error('Video load failed for thumbnail'))
    ...
    const duration = Math.max(0, Math.min(1, Math.floor(videoEl.duration) || 0))
    ...
    const thumbnailFile = new File([thumbnailBlob], `thumb-${Date.now()}.png`, {
      type: 'image/png',
    })
